...
function showBaseModal({
  ...
  title: string;
  modalBody: HTMLElement | JQuery<HTMLElement>;
  modalFooter?: HTMLElement | JQuery<HTMLElement>;
}) {
  const modal = bootstrap.Modal.getOrCreateInstance($("#modal__base"));
  if (title) $("#modal__base .modal-header .modal-title").text(title);
  $("#modal__base .modal-content").append(modalBody);
...
function populateTableEmployees() {
  api
    .get<Array<Employee>>("/employees/master")
    .then((res) => {
...
$<HTMLFormElement>("#modal__insertEmployee form").on("click", (e) => {
